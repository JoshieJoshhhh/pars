...
try {
    ...
    $result = $conn->query("SELECT MAX(orderID) + 1 AS nextID FROM transactions");
    $row = $result->fetch_assoc();
    $transID = $row['nextID'] ?? 1; // fallback to 1 if no rows exist
    /////////////////////////////////////
    
    $transactionQuery = "INSERT INTO transactions (orderID, cashierID, orderSummary, totalAmount, paymentMethod, status, created_at) VALUES (?, ?, ?, ?, ?, ?, NOW())";//added order ID and another "?"
    $transactionStmt = $conn->prepare($transactionQuery);
    $orderSummary = json_encode($cartItems);
    $status = 'Completed';
    $transactionStmt->bind_param('iisdss', $transID, $_SESSION['userID'], $orderSummary, $totalAmount, $paymentMethod, $status);///added another i for order ID
