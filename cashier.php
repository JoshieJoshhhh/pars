...
<body>
  ...
  <script>
    ...
    async function loadCloseoutSummary() {
          try {
              // Fetch all orders again to ensure the closeout summary is up-to-date
              const response = await fetch('db/orders_get.php');
              const data = await response.json();

              if (data.status === 'success') {
                  orders = [...(data.pending || []), ...(data.completed || [])]; // Update the global orders array

                  let totalOrders = orders.length;
                  let grossSales = 0;
                  let totalDiscounts = 1000; // Assuming no discount field in table, keeping as 0

                  orders.forEach(order => {
                      grossSales += parseFloat(order.totalAmount || 0);
                      // If 'Order Summary' JSON contained a discount, you'd parse it here
                      // Example: const orderSummary = JSON.parse(order.OrderSummary);
                      // if (orderSummary.discount) totalDiscounts += parseFloat(orderSummary.discount);
                  });

                  let netSales = grossSales - totalDiscounts;

                  // Update the DOM elements
                  document.getElementById('summaryOrders').textContent = totalOrders;
                  document.getElementById('summaryGross').textContent = currency(grossSales);
                  document.getElementById('summaryDiscounts').textContent = currency(totalDiscounts); // Currently ₱0.00
                  document.getElementById('summaryNet').textContent = currency(netSales);
              } else {
                  console.error('Failed to load orders for closeout:', data.message);
                  showToast('Failed to load closeout summary orders', 'error');
              }
          } catch (error) {
      ...
      const closeoutNavItem = document.querySelector('[data-section="CloseoutForm"]'); // <--- ADD THIS LINE


      // <--- ADD THIS NEW BLOCK FOR CLOSEOUT
      if (closeoutNavItem) {
          closeoutNavItem.addEventListener('click', function() {
              loadCloseoutSummary();
          });
      }
      ...
      async function loadOrders(){
        try {
          ...
          showToast('Error loading orders', 'error');
            ...
            // 1. Get the element you want to attach the click event to
      const ordersNavItem = document.querySelector('[data-section="OrdersForm"]');

      // 2. Check if the element was found
      if (ordersNavItem) {
          // 3. Attach a 'click' event listener
          ordersNavItem.addEventListener('click', function() {
              // 4. Call your function inside the event handler
              loadOrders();
              
              // OPTIONAL: Add active class/logic for navigation UI
              // Example: setActiveSection('OrdersForm'); 
          });
      }
    ...
    // ... (omitted: check for orders.length)

    orders.forEach(o => {
        const tr = document.createElement('tr');
        
        // --- 1. Create the TR's Inner Cells (excluding Items) ---
        // This keeps the HTML safe for all other cells
        tr.innerHTML = `<td>${escapeHtml(o.orderID || 'N/A')}</td>
                        <td id="items-${o.orderID}"></td> <td>${currency(o.totalAmount || 0)}</td>
                        <td>${escapeHtml(o.status || 'pending')}</td>
                        <td>${escapeHtml(o.orderID || 'N/A')}</td>`;
                        
        tbody.appendChild(tr); // Append the row first

        // --- 2. MANUALLY RENDER THE 'ITEMS' CELL ---
        const itemsCell = document.getElementById(`items-${o.orderID}`);
        const itemsArray = (o.items || 'No items').split('\n'); // Split the PHP string by the newline

        itemsArray.forEach((itemText, index) => {
            // Escape the text of the item for security
            const escapedText = escapeHtml(itemText);
            
            // Create a text node for the escaped item description
            itemsCell.appendChild(document.createTextNode(escapedText));

            // Add a <br> tag after every item, except the last one
            if (index < itemsArray.length - 1) {
                itemsCell.appendChild(document.createElement('br'));
            }
        });
    });
}
